      ...
      <div className="text-center mb-36">
        <p className="mt-2 text-sm font-mono">Currently selected page:{ page +1 }</p>
            {[...Array(pages).keys()].map((number) => (
              <button onClick={()=>setPage(number)} key={number } className="btn btn-xs btn-outline mr-3 border-b-2">{number +1}</button>
            ))}
        
        <select onChange={(e)=>setSize(e.target.value)} defaultValue={size} className="border rounded-md border-gray-500 pb-1">
